...
public class PerformaceTest {
    ...
    public static int TOTAL_RECORD = 200*10000;
    ...
    public static void main(String[] args) throws Exception {
        PerformaceTest performaceTest = new PerformaceTest();
        ...
        try (Ignite ignite = Ignition.start("examples/config/zookeeper-config.xml")) {
            ...
            performaceTest.createTable(cache);
            startTime = new Date();
            ...
            for(int i=1;i<=CONNECTOR_NUM;i++){
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        performaceTest.insertData(cache, sn, seq);
                    }
                }).start();
            }
    ...
    public void createTable(IgniteCache<?, ?> cache  ){
        ...
        // drop table
//        cache.query(new SqlFieldsQuery("drop TABLE if exists tran")).getAll();
...
//                cache.query(new SqlFieldsQuery("CREATE INDEX on Person (city_id)")).getAll();
    ...
    public void insertData( IgniteCache<?, ?> cache,  int sn, IgniteAtomicSequence seq  ){
